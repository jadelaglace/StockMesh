...
- Open the workbench in Simplified Chinese by default and let the user switch between Simplified Chinese and English with a locally retained preference. Localize all owned interface copy and offer locale-specific presentation copy for the repository-owned public synthetic example, while keeping imported evidence, user input, private/real records, identifiers, and stored Domain records verbatim.
