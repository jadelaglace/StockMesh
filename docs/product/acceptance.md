...
| AC-27 | The Web workbench can switch its owned interface chrome and date/status presentation between English and Simplified Chinese, retains that preference locally, and keeps evidence, user input, Node/Pawn labels, modeled branch content, identifiers, and stored Domain records unchanged. | Language switching is partial or inaccessible, resets unexpectedly, mutates data, or presents translated source/model content as the original. |
